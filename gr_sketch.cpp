...
/*
 * Nov. 24th 2016, Minai Yamamoto: For GR-CITRUS, fixed bug in folder
 * search specification for mp3 file. Fixed to close files that were not
 * closed  for GR-CITRUS.
 */
...
void printDirectory(const char *parentDir, File dir, int numTabs)
{
char fpath[1024] = "";
File entry;
	while(true)
	{
		...
		if(!entry)
		{
			strcpy(fpath, "no file in '");
			strcat(fpath, parentDir);
		...
		if(entry.isDirectory())
		{
			...
			printDirectory(parentDir, entry, numTabs + 1);
		}
		else
		{
			strcpy(fpath, parentDir);
			for(uint8_t i=0; i< numTabs; i++)
			{
				strcat(fpath, path[i]);
				strcat(fpath, "/");
...
void loop()
{
//const char *parentDir = "/ABC/";
const char *parentDir = "/";

	File root = SD.open(parentDir);
	printDirectory(parentDir, root, 0);
	Serial.println("done!");
